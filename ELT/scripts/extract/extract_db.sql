...
WHERE DATE_TRUNC('day', c.company_update_time_stamp) = DATE_TRUNC('day', CURRENT_DATE - 1)
    AND DATE_TRUNC('month', c.company_update_time_stamp) = DATE_TRUNC('month', CURRENT_DATE - 1)
	AND DATE_TRUNC('year', c.company_update_time_stamp) = DATE_TRUNC('year', CURRENT_DATE - 1);